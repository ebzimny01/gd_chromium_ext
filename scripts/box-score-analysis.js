...
(async () => {
    ...
    if (url.startsWith(main_schedule_page) || url.startsWith(teamId_schedule_page)) {
        ...
        if (url.startsWith(teamId_schedule_page)) {
            ...
            gdanalyst_team_schedule_page = await getGDAnalystTeamSchedulePage(teamId);
        };
        ...
        let html_to_insert = parser.parseFromString(`<h3><a href="${gdanalyst_team_schedule_page}" target="_blank" style="color:blue">The Next Guess Game Analysis</a></h3>`, "text/html");
        ...
        r.forEach(element => {
            ...
            let boxscoreurl = `https://www.thenextguess.com/gdanalyst/pbp?gameid=${boxscoreid[1]}`;
